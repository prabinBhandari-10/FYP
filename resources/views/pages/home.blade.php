...
<div style="display: grid; grid-template-columns: 280px minmax(0, 1.4fr) minmax(0, 320px); gap: 20px; margin-bottom: 20px;">
        ...
        <section class="card">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                @if ($isAdmin)
                    ...
                    <a href="{{ route('reports.lost.create') }}" class="btn btn-primary" style="justify-content: center;">📝 Report Lost</a>
                    <a href="{{ route('reports.found.create') }}" class="btn btn-outline" style="justify-content: center;">✅ Report Found</a>
                @else
                    @if (auth()->check())
                        <a href="{{ route('reports.lost.create') }}" class="btn btn-primary" style="justify-content: center;">📝 Report Lost</a>
                        <a href="{{ route('reports.found.create') }}" class="btn btn-outline" style="justify-content: center;">✅ Report Found</a>
    ...
    <aside class="sticky-panel" style="display: grid; gap: 14px;">
        @auth
            @if (!$isAdmin)
                <article class="card card-soft">
                    <h3 style="font-size: 16px; margin-bottom: 10px; color: var(--text-main);">📊 Your Activity</h3>
                    @php
                        $userReportsCount = auth()->user()->reports()->count();
                        $userClaimsCount = auth()->user()->claims()->count();
                        $userApprovedCount = auth()->user()->claims()->where('status', 'approved')->count();
                    @endphp
                ...
                <article class="card card-soft">
                    ...
                    <div style="display: grid; gap: 8px;">
                        ...
                        <a href="{{ route('claims.index') }}" class="btn btn-outline" style="font-size: 13px; justify-content: center;">My Claims</a>
